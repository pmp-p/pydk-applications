...
#            win.requestProperties(props)
    ...
    def openMainWindow(self, *args, **kw):
        ...
        if self.win != None:
            pdb("985: discarded close of",self.win)
            if 0:
            ...
            # Open a new window.
            embed.log("997 - showbase")
            self.openWindow(*args, **kw)
            embed.log(f"999 - showbase {self.win}")

            if self.win == None:
                self.win = oldWin
                pdb("1000: failure, re using oldWin, success to 0",self.win)
